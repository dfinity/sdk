...
At both times, the canister must be stopped with `dfx canister stop`. You can use the `--force` flag to override this.

### Basic usage

```bash
dfx canister snapshot create <canister> [--replace <replace>] [--force]
```
...
| `-f`, `--force`       | Force snapshot creation even if the canister is running. Not recommended unless you know what you're doing. |
...
```sh
dfx canister snapshot load <canister> <snapshot> [--force]
```
...
| `-f`, `--force` | Force snapshot loading even if the canister is running. Not recommended unless you know what you're doing. |
