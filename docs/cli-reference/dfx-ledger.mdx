...
| `--expected-allowance <cycles>`     | The number of previously approved ICP. See [ICRC-2 standard](https://github.com/dfinity/ICRC-1/blob/main/standards/ICRC-2/README.md) for details. |
...
Use the `dfx ledger transfer-from` command to transfer ICP from the approver princiapl to another principal. Please check the [ICRC-2 standard](https://github.com/dfinity/ICRC-1/tree/main/standards/ICRC-2) for more information about token `approve` and `transfer-from`.
