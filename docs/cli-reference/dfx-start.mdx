...
If you run the local development environment in the background, however, be sure to stop the local development environment before uninstalling or reinstalling the `dfx` execution network by running the following command:
