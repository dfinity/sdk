...
Use the `dfx start` command to start a local development network and web server processes. This command must be run before you can deploy canisters to the local development network and test your dapps during development.

By default, all local dfx projects will use PocketIC to create this local development network. You can run `dfx start` and `dfx stop` from any directory.  See [local server configuration](#local-server-configuration) and [project-specific local networks](#project-specific-local-networks) below for exceptions.
...
| Flag                     | Description                                                                                                                                                                                                                                             |
|--------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `--background`           | Starts the local development network and web server processes in the background and waits for a reply before returning to the shell.                                                                                                         |
| `--clean`                | Starts the local development network and web server processes in a clean state by removing checkpoints from your project cache. You can use this flag to set your project cache to a new state when troubleshooting or debugging.            |
...
You can start the local development network and web server processes in the current shell by running the following command:

``` bash
dfx start
```

If you start the local development network in the current shell, you need to open a new terminal shell to run additional commands. Alternatively, you can start the local development network in the background by running the following command:

``` bash
dfx start --background
```

If you run the local development network in the background, however, be sure to stop the local development network before uninstalling or reinstalling the `dfx` execution network by running the following command:

``` bash
dfx stop
```

You can view the current process identifier (`pid`) for the local development network process started by `dfx` by running the following command:
...
`dfx start` manages a PocketIC server that provides a local development network. Running `dfx deploy` and other commands will manage canisters on this local network in the same way that `dfx deploy --network ic` deploys projects to the mainnet.
