...
. Store a path to your latest system dfx as a variable. (This requires having the latest `dfx` installed)
+
[source, bash]
----
...
. Create and switch to a new directory to release from
+
[source, nix-shell]
----
pushd $(mktemp -d)
cp -R $agent_js_rc/. ./
// tar -xf $agent_js_rc_npm_packed
// cd package
----
