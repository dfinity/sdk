...
+
Remind them to add information about _features and fixes_ for release notes to their issues or PRs and to apply the changelog label to have the information included in the release notes.

=== Promoting a released version to Latest

Before promoting, check whether any features or fixes require documentation. 
In some cases, we might promote a release without updating any documentation.
However, it would be unusual to promote without any release notes.

. Check whether any issues have the changelog label.
. Check with DX product (sometimes referred to as Lisa) to hold back promotion because release notes or documentation are not ready for public consumption.
