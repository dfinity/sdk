...
We will implement the canister in Motoko using existing stable memory data structures such as RBTree.
...
** Blobs storage in files implies management of those files and allocation of their filenames
* Performance considerations:
** Retrieval requires reading the content out of stable memory into a blob before
returning it
*** (since content is not stored in contiguous memory)
** General overhead of traversing logical directory paths
** Depending on the number of asset content blobs we expect an asset canister to hold,
we might have to split up the files that store content into subdirectories.

===== FAT32 implementation notes (rejected)

These are notes for how we could have implemented this in a Rust canister using a block of stable memory.
...
service: {
  ...
  list: () -> (vec AssetInfo) query;

  // allocate space for content
  create_blobs: (vec TotalLength) -> (vec BlobId);

  // upload part of a blob's content
  write_blob(BlobId, Offset, Contents) -> ();
