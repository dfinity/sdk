...
service: {
  ...
  get: (record {
    ...
    content: blob; // may be the entirely of the content, or just chunk index 0
    content_type: text;
    content_encoding: text,
    chunk_count: nat
  }) query;
  ...
  // only if set_encoding_chunk() has not been called for any of those encodings recently.
  ...
  set_encoding_chunk: (record { encoding_id: EncodingId; index: nat; content: blob }) -> ();
