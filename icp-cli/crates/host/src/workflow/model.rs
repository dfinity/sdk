...
struct NodeYaml {
    ...
    #[serde(default)]
    value: Option<String>, // for ConstNode
    #[serde(default)]
    inputs: HashMap<String, String>, // input name → source node name
}
...
impl Workflow {
    ...
    pub fn from_string(yaml: &str) -> Self {
        let wf: WorkflowYaml = serde_yaml::from_str(yaml).expect("Failed to parse YAML");
        Self::new(wf)
    }
...
"#;
        ...
        let _workflow = Workflow::from_string(yaml);
    }
...
"#;
        ...
        let workflow = Workflow::from_string(yaml);
