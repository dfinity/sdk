...
The native replica is no longer bundled with dfx; dfx only uses PocketIC for local networks. Accordingly the `dfx start --replica` and `dfx info replica-port` flags no longer function. See the [migration guide](./docs/migration/dfx-0.27.0-migration-guide.md) for more information.
