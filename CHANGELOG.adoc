...
Breaking change: Canister commands and `dfx deploy` will now call as the user, rather than the user's wallet. The `--no-wallet` flag is thus removed as its behavior is the default.

When upgrading existing canisters, use the `--wallet` flag in conjunction with `dfx identity get-wallet` in order to restore the old behavior.
