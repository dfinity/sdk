...
This would break compatibility with `rust` and `custom`, which currently use `CANISTER_CANDID_{canister}`. For now, this naming scheme is available
for compatibility reasons, but will be removed in 0.10.
