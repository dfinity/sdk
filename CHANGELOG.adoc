...
You will need to upgrade each of your existing canisters to work with the new system.  To do so, execute the following command for each of your canisters (substituting `canister_name` for your own):
[source, bash]
----
dfx canister --wallet "$(dfx identity get-wallet)" update-settings <canister_name> --add-controller "$(dfx identity get-principal)"
----
Or, if you're upgrading a canister on the IC:
[source, bash]
----
dfx canister --network ic --wallet "$(dfx identity --network ic get-wallet)" update-settings <canister_name> --add-controller "$(dfx identity get-principal)"
----
