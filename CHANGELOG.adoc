...
Enables passing large arguments that cannot be passed directly in the command line using the `--argument-file` flag. For example `dfx canister call --argument-file ./my/argument/file.txt my_canister_name greet` or `echo '( null )' | dfx canister call --argument-file -`.
