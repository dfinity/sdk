...
When working with existing canisters, use the `--wallet` flag in conjunction with `dfx identity get-wallet` in order to restore the old behavior. To upgrade an existing canister to work seamlessly with the new system, the following command should be used (substituting `canister_name` for your own):
[source, bash]
----
