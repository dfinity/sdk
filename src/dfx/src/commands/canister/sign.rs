...
use crate::util::{arguments_from_file, blob_from_arguments, get_candid_type, CandidSource};
...
pub async fn exec(
    ...
    let method_type =
        maybe_candid_path.and_then(|path| get_candid_type(CandidSource::Path(&path), method_name));
