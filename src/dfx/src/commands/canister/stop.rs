...
async fn stop_canister(env: &dyn Environment, agent: &Agent, canister_name: &str) -> DfxResult {
    ...
    mgr.stop_canister(create_waiter(), &canister_id)
        .await
...
pub fn exec(env: &dyn Environment, args: &ArgMatches<'_>) -> DfxResult {
    ...
    if let Some(canister_name) = args.value_of("canister_name") {
        runtime.block_on(stop_canister(env, &agent, &canister_name))?;
        Ok(())
