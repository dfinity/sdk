...
pub fn exec(env: &dyn Environment, opts: GenerateRemoteBindingOpts) -> DfxResult {
    ...
    for canister in canister_pool.get_canister_list() {
        ...
        if let Some(candid) = info.get_remote_candid() {
            let src_optional: Option<String> = info.get_extra_optional("src")?;
            if let Some(src) = src_optional {
                ...
                if let Some(bindings_string) = bindings {
                    std::fs::write(&src_path, &bindings_string)?;
                    info!(
                        log,
