...
fn get_config(
    ...
    let root = get_root(env, args, &base)?;
...
fn get_ip(args: &ConfigDefaultsBootstrap, base: &ConfigDefaultsBootstrap) -> IpAddr {
    ...
    args.ip.unwrap_or_else(|| base.ip.unwrap_or(default))
}
...
fn get_port(args: &ConfigDefaultsBootstrap, base: &ConfigDefaultsBootstrap) -> u16 {
    let default = 8081;
    args.port.unwrap_or_else(|| base.port.unwrap_or(default))
}
...
fn get_providers(args: &ConfigDefaultsBootstrap, base: &ConfigDefaultsBootstrap) -> Vec<Url> {
    ...
    args.providers
        .clone()
        .unwrap_or_else(|| base.providers.clone().unwrap_or(default))
}
...
fn get_root(
    ...
    Ok(args
        .root
        .clone()
        .unwrap_or_else(|| base.root.clone().unwrap_or(default)))
}
...
fn get_timeout(args: &ConfigDefaultsBootstrap, base: &ConfigDefaultsBootstrap) -> u64 {
    let default = 30;
    args.timeout
        .unwrap_or_else(|| base.timeout.unwrap_or(default))
}
