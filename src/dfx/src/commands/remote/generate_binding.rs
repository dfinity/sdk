        ...
        if let Some(candid) = info.get_remote_candid() {
            ...
            if let Some(main) = main_optional {
                ...
                let bindings = if extension == "mo" {
                    Some(candid_parser::bindings::motoko::compile(&type_env, &did_types))
                    ...
                    Some(candid_parser::bindings::javascript::compile(&type_env, &did_types))
                } else if extension == "ts" {
                    Some(candid_parser::bindings::typescript::compile(&type_env, &did_types))
                } else {
                    info!(
                        log,
