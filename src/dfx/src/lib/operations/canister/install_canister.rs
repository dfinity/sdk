...
"#;
    ...
    if matches!(mode, InstallMode::Reinstall | InstallMode::Upgrade) {
        ...
        if let Some(candid) = candid {
            ...
            let new_type = opt_new.expect("Generated did file should contain service interface");
            let (env2, opt_old) = check_candid_file(&deployed_path)?;
            let old_type = opt_old.expect("Deployed did file should contain service interface");
