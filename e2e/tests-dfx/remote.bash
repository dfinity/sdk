...
@test "canister call and sign" {
  ...
  assert_eq '("initial data in the remote canister")' "$stdout"
  ...
  assert_eq '(record { a = "A query by principal"; b = "B query by principal" })' "$stdout"
  ...
  assert_eq '(record { a = "A default by principal"; b = "B default by principal" })' "$stdout"
...
@test "all commands with --all skip remote canisters" {
  ...
  assert_command dfx canister call remote read --identity alice --network actuallylocal
  assert_eq '("this is data in the remote canister")' "$stdout"
  ...
  assert_command dfx canister call basic read_remote --network actuallylocal
  assert_eq '("altered data (by canister id) in the remote canister")' "$stdout"
  ...
  assert_command dfx canister call basic read_remote --network actuallylocal
  assert_eq '("altered data (by canister name) in the remote canister")' "$stdout"
  ...
  assert_eq '"'"$(dfx canister id basic --network actuallylocal)"'"' "$stdout"

  assert_command jq .remote canister_ids.json
  assert_eq "null" "$stdout"
...
@test "build + install + call -- remote" {
  ...
  assert_command dfx canister call basic read_remote --network actuallylocal
  assert_eq '("data altered by canister id in the remote canister")' "$stdout"

  # We can also call through the canister name
  assert_command dfx canister call remote write '("data altered by canister name in the remote canister")' --network actuallylocal
  assert_command dfx canister call basic read_remote --network actuallylocal
  assert_eq '("data altered by canister name in the remote canister")' "$stdout"
