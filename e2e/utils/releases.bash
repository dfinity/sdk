...
get_from_latest_release_tarball() {
    local file_path=$1
    local destination=$2
    local tarball_url=$(curl -sL "https://api.github.com/repos/dfinity/sdk/releases/latest" | jq -r ".tarball_url")

    local temp_dir=$(mktemp -d)
    curl -sL "$tarball_url" -o "$temp_dir/release.tar.gz"

    tar -xzf "$temp_dir/release.tar.gz" -C "$temp_dir" "*/$file_path"
    extracted_file=$(find "$temp_dir" -type f -name $(basename "$file_path"))
