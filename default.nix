...
let
  packages = rec {
    ...
    #
    # This script will be executed by DFINITY's hail-based
    # Continuous Deployment system. That system will also set the
    # correct AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY and
    # DFINITY_DOWNLOAD_BUCKET environment variables.
    publish =
      let
          ...
          pkgs.lib.writeCheckedShellScriptBin "activate" [] ''
            ...
            s3cp() {
              ...
              ${pkgs.awscli}/bin/aws s3 cp \
                "$pkg/$file" "s3://$DFINITY_DOWNLOAD_BUCKET/$dstDir/$file" \
            ...
            # TODO: I don't like not caching install.sh. Consider configuring the S3 bucket as a
            # website and turning install.sh into an uncached (or shortly cached) redirect to a
